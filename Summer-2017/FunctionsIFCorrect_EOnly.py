...
'''
def updateVoltageAndThreshold(neuron,dt,injectCurrentExternal,lambdaValue):
    injectCurrent = injectCurrentExternal
    if neuron.population == "Inhibitory":
        injectCurrent = injectCurrentExternal*0.8
    k_1=(-(neuron.V_m -neuron.Vrest)+injectCurrent)/neuron.tau
    k_2=(-(neuron.V_m+k_1/2.0*dt -neuron.Vrest)+injectCurrent)/neuron.tau
    k_3=(-(neuron.V_m+k_2/2.0*dt-neuron.Vrest)+injectCurrent)/neuron.tau
    k_4=(-(neuron.V_m + k_3*dt-neuron.Vrest)+injectCurrent)/neuron.tau
    neuron.V_m=neuron.V_m+dt/6.0*(k_1+2.0*k_2+2.0*k_3+k_4)

    k_1_t=dt*lambdaValue*(neuron.originalThresh-neuron.threshold)
    k_2_t=dt*lambdaValue*(neuron.originalThresh-(neuron.threshold + k_1_t/2.0))
    k_3_t=dt*lambdaValue*(neuron.originalThresh-(neuron.threshold + k_2_t/2.0))
    k_4_t=dt*lambdaValue*(neuron.originalThresh-(neuron.threshold+k_3_t))
    neuron.threshold=neuron.threshold+1.0/6*(k_1_t+2*k_2_t+2*k_3_t+k_4_t)

def updateVoltageAndThreshold(neuron,dt,injectCurrentExternal,lambdaValue):
    V_m = neuron.V_m
    Vrest = neuron.Vrest
    tau = neuron.tau
    originalThresh = neuron.originalThresh
    threshold = neuron.threshold

    injectCurrent = injectCurrentExternal
    if neuron.population == "Inhibitory":
        injectCurrent = injectCurrentExternal*0.8
    k_1=(-(V_m -Vrest)+injectCurrent)/tau
    k_2=(-(V_m+k_1/2.0*dt -Vrest)+injectCurrent)/tau
    k_3=(-(V_m+k_2/2.0*dt-Vrest)+injectCurrent)/tau
    k_4=(-(V_m + k_3*dt-Vrest)+injectCurrent)/tau
    neuron.V_m=V_m+dt/6.0*(k_1+2.0*k_2+2.0*k_3+k_4)

    k_1_t=dt*lambdaValue*(originalThresh-threshold)
    k_2_t=dt*lambdaValue*(originalThresh-(threshold + k_1_t/2.0))
    k_3_t=dt*lambdaValue*(originalThresh-(threshold + k_2_t/2.0))
    k_4_t=dt*lambdaValue*(originalThresh-(threshold+k_3_t))
    neuron.threshold=threshold+1.0/6*(k_1_t+2*k_2_t+2*k_3_t+k_4_t)
'''
